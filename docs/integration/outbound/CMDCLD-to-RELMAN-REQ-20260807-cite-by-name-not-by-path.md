# CMDCLD-to-RELMAN-REQ-20260807 — Cite the skill by name, never by path

*From: CmdCLD (`CMDCLD`) to release-manager (`RELMAN`) — 2026-08-07. Assent-only. Second
...
In release terms: a path is a citation that has not been published. It resolves on the
machine that authored it and nowhere else, which is why the skill name and the repository
URL are the only two forms this protocol now accepts.
...
`CMDCLD-to-RELMAN-REQ-20260807-cite-by-name-not-by-path-ack.md` in your `outbound/`.
