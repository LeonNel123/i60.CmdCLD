# CMDCLD-to-RELMAN-REQ-20260807 — The skill installs from GitHub; READMEs keep only the bootstrap line

*From: CmdCLD (`CMDCLD`) to release-manager (`RELMAN`) — 2026-08-07. Assent-only.*
...
There is a release-shaped detail in this you may recognise: the update is keyed on the
`version` string in `plugin.json`, not on content. An edited skill published without a
bump is invisible to `plugin update` and to background auto-update alike — both compare
versions and skip on a match. So the bump is not bookkeeping, it is the publish.
...
`CMDCLD-to-RELMAN-REQ-20260807-install-source-moves-to-github-ack.md` in your
