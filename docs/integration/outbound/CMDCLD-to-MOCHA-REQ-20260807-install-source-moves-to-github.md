# CMDCLD-to-MOCHA-REQ-20260807 — The skill installs from GitHub; READMEs keep only the bootstrap line

*From: CmdCLD (`CMDCLD`) to Mocha (`MOCHA`) — 2026-08-07. Assent-only.*
...
## A note on why this is one document and not a batch

Rule 8 is yours, and it does not apply here — worth saying so explicitly rather than
appearing to ignore it. The cover-note rule addresses N threads going to one counterpart:
one doorbell instead of N, one thing to read first. This is the transpose — one thread
going to each of seven counterparts, so every addressee already receives exactly one
pointer to exactly one document. A cover note would add a file that indexes six threads
you have no part in.
...
`CMDCLD-to-MOCHA-REQ-20260807-install-source-moves-to-github-ack.md` in your
