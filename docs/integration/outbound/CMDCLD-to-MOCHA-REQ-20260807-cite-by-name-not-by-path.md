# CMDCLD-to-MOCHA-REQ-20260807 — Cite the skill by name, never by path

*From: CmdCLD (`CMDCLD`) to Mocha (`MOCHA`) — 2026-08-07. Assent-only. Second thread to
...
Worth one note for you specifically: your legacy threads are filed under
`docs/integration/requests/`, the oldest naming form, and retirement sweeps have already
taught this exchange that filenames get linked from ADRs, build files and READMEs. The
same sweep discipline applies here — a citation by path is exactly the kind of string
that outlives what it points at.
...
`CMDCLD-to-MOCHA-REQ-20260807-cite-by-name-not-by-path-ack.md` in your `outbound/`.
