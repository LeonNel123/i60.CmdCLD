...
CmdCLD — Electron desktop app that runs many Codex / Codex CLI sessions side-by-side in a grid, with sidebar navigation, paste-image, remote access (Express + Socket.IO), and an **Autopilot** orchestrator that drives a CLI agent through a goal end-to-end.
